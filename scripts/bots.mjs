/**
 * Eight bidders driving the auction from this laptop.
 *
 ...
 *
 *   npm run bots              8 bots, join + bid on whatever is open
 *   npm run bots -- 4         fewer bots
 *
...
import { CRORE } from '../src/lib/format.mjs'
...
const count = Math.min(Number(process.argv[2]) || 8, NAMES.length)
...
console.log(`\n  ${count} bidders on ${CONTRACT}\n`)

// --- fund + join -------------------------------------------------------------
const master = accountFor(masterKey)
const masterWallet = walletFor(masterKey)
let masterNonce = await publicClient.getTransactionCount({ address: master.address, blockTag: 'latest' })
...
for (const bot of bots) {
  bot.nonce = await publicClient.getTransactionCount({ address: bot.account.address, blockTag: 'latest' })
  const [entityId] = await publicClient.readContract({
    address: CONTRACT, abi: BIDBLITZ_ABI, functionName: 'purseOf', args: [bot.account.address],
  })
  ...
  if (Number(entityId) === 0) {
    ...
    try {
      await bot.wallet.writeContract({
        address: CONTRACT, abi: BIDBLITZ_ABI, functionName: 'joinSquad', args: [squad],
        gas: GAS.joinSquad, nonce: bot.nonce++, ...feeParams(),
      })
      bot.entityId = squad
...
while (true) {
  let s
  try {
    s = await publicClient.readContract({ address: CONTRACT, abi: BIDBLITZ_ABI, functionName: 'state' })
  } catch {
  ...
  if (lotId !== lastLot) {
    lastLot = lotId
    console.log(`\n  LOT #${lotId}  ${s.lname}`)
    for (const b of bots) b.nonce = await publicClient.getTransactionCount({ address: b.account.address, blockTag: 'latest' })
  }
  ...
  for (const bot of bots) {
    ...
    bot.lastAct = t
    const step = BigInt(1 + Math.floor(Math.random() * 3)) * CRORE
    ...
    try {
      await bot.wallet.writeContract({
        address: CONTRACT, abi: BIDBLITZ_ABI, functionName: 'placeBid',
        args: [BigInt(lotId), amount],
        gas: GAS.placeBid, nonce: bot.nonce++, ...feeParams(),
      })
      bidCount++
      console.log(`  ${bot.name.padEnd(9)} bid ${Number(amount / CRORE)} Cr   (${bidCount} total)`)
    } catch (err) {
